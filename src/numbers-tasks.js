...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const first = Math.abs(x2 - x1);
  const second = Math.abs(y2 - y1);
  const gipot = first ** 2 + second ** 2;
  return Math.sqrt(gipot);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const xVector = x1 * x2 + y1 * y2;
  const lVector = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = xVector / lVector;
...
function getSumOfDigits(num) {
  ...
  const arrSum = numArr.reduce((i, sum) => {
    return i + sum;
  }, 0);
  return arrSum;
}
